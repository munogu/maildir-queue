...
class Queue {
  ...
  add (payload = {}, envelope = {}) {
    ...
    return write(tmp, item).then(() => {
      debug('written into tmp: ' + item.id)
      return fs.rename(tmp, path.join(this.new, filename))
    }).then(() => {
      debug('moved to new: ' + item.id)
      return item
    })
  }
  pop (handler) {
    return this.list().then(items => detectSeries(items, file => {
      ...
      return result.then(returned => {
        ...
        // expose item in error
        err.item = file.item
        // check retries
        if (this.opts.retries >= 0 && file.item.retry >= this.opts.retries) {
          return fs.unlink(file.filename).then(() => Promise.reject(err))
        }
        // requeue file
        let now = new Date()
        file.item.updatedAt = now
        file.item.retry++
        return write(file.filename, file.item).then(() => {
          // rename file
